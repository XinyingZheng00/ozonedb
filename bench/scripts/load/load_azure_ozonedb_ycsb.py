...
from pyozonedb_data_cache import *

# Logging setup
# todo: redesign the configuration file for the cloud storage, client has limited memory size, so we cannot make the file too large
# client will have much less connection capacity
    ...
    for op_cnt in map(str, operation_cnts):
        for key_size in key_sizes:
            for workload in workloads:
                ...
                server_exec(node, "rm -rf " + insert_result, tmux_session="ozonedb_load")
                
                cached_data_path = f"cached-data-{db}-{key_size}-workload{workload}-{record_cnt}".lower()
                delete_blobs_in_directory(get_container_client(blob_client, "ycsb-data"), cached_data_path)
                
                gen_config_command = f"python3 {script_path}/generate_config_for_ozonedb.py --new_dir {cached_data_path}/ --is_local 0"
                server_exec(node, gen_config_command, tmux_session="ozonedb_load")
                command = ["python3", "bin/ycsb", "load", db, '-threads', "64", "-s", "-P", workload_path, "-p", f"shared_config=$OZONEDB_HOME/src/config/cloud/shared_config_rocksdb.json", f"-p statusinterval=1 2>&1 | tee -a {insert_result}"]
                ycsb_t_start.wait()
