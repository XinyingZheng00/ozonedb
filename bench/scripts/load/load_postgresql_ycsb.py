...
#psql -h ozonedb-postgresql-server.postgres.database.azure.com -p 5432  -U Xinying -d ycsb
...
def load_ycsb(node, i, ycsb_t_start, config):    
    ...
    server_exec(node, "tmux new -s postgresql_load -d")
    server_exec(node, f"mkdir -p {result_path}", tmux_session="postgresql_load")
    server_exec(node, f"cd {ycsb_path}", tmux_session="postgresql_load")
    ...
    for op_cnt in map(str, operation_cnts):
        for key_size in key_sizes:
            for workload in workloads:
                ...
                server_exec(node, gen_command, tmux_session="postgresql_load")
                ...
                server_exec(node, "rm -rf " + insert_result, tmux_session="postgresql_load")
                
                cached_data_path = f"cached_data_{db}_{key_size}_workload{workload}_{record_cnt}".lower()
                table_name = "usertable"
                clear_table(ip, port, "ycsb", table_name, username, password)
                
                command = ["python3", "bin/ycsb", "load", 'jdbc','-threads', "64", 
                           "-p", "db.batchsize=5000",
                           "-p", "jdbc.batchupdateapi=true",
                           "-s", "-P", workload_path, "-P", "$OZONEDB_HOME/bench/scripts/postgres.properties", f"-p statusinterval=1 2>&1 | tee -a {insert_result}"]
                ycsb_t_start.wait()
                server_exec(node, " ".join(command), tmux_session="postgresql_load")
                # copy_table(
                #     ip,
                #     port,
                #     "ycsb",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load ycsb-t data on Azure VMs and Postgresql")
