...
def create_blob_service_client(config):
    """Blob client for cache cleanup; key from env/yaml or Azure ARM."""
...
def load_ycsb(node, i, ycsb_t_start, config, blob_client, ssh_identity_path="~/.ssh/id_rsa"):
    ...
    server_exec(node, f"cd {ycsb_path}", tmux_session="ozonedb_load", ssh_identity_path=ssh_identity_path)
    ...
    for each_record_cnt in record_cnt:
        ...
        for each_key_size in key_sizes:
            ...
            server_exec(node, gen_command, tmux_session="ozonedb_load", ssh_identity_path=ssh_identity_path)
            ...
            ycsb_t_start.wait()
            server_exec(node, " ".join(command), tmux_session="ozonedb_load", ssh_identity_path=ssh_identity_path)
...
def download_results(node, config, ssh_identity_path="~/.ssh/id_rsa"):
    ...
    for each_key_size in workload_config["key_size"]:
        for each_record_cnt in workload_config["record_cnt"]:
            ...
            download_file_from_server(node, remote_insert_result, local_result_path, ssh_identity_path=ssh_identity_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load YCSB on cloud VMs (Azure or CloudLab) with Ozonedb")
    ...
    # prepare vm list, cloudlab verified
    ...
    for each_record_cnt in record_cnt:
        ...
        for each_key_size in key_sizes:
            for server in server_list:
                db_name = "ozonedb"
                result_file_insert = os.path.join(result_path, f'{each_key_size}-{each_record_cnt}-insert-{db_name}_t{threads}.result')
                print(result_file_insert)
                server_exec(server, "source ~/.profile && rm -rf " + result_file_insert, ssh_identity_path=ssh_identity_path)
