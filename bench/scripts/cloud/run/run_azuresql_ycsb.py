...
def run_ycsb(node, i, ycsb_t_start, config):
    ...
    server_exec(node, "tmux new -s azuresql_run -d")
    server_exec(node, f"mkdir -p {result_path}", tmux_session="azuresql_run")
    server_exec(node, f"cd {ycsb_path}", tmux_session="azuresql_run")
    ...
    for op_cnt in map(str, operation_cnts):
        for key_size in key_sizes:
            for workload in workloads:
                ...
                server_exec(node, gen_command, tmux_session="azuresql_run")
                ...
                server_exec(node, "rm -rf " + run_result, tmux_session="azuresql_run")
                
                cached_data_path = f"cached_data_{db}_{key_size}_workload{workload}_{record_cnt}".lower()
                table_name = "usertable"
                copy_table(conn_str,cached_data_path,table_name)
                command = ["python3", "bin/ycsb", "run", 'jdbc','-threads', f'{threads}', "-s", "-P", workload_path, "-P", "$OZONEDB_HOME/bench/scripts/config/azuresql.properties", f"-p statusinterval=1 2>&1 | tee -a {run_result}"]
                
                ycsb_t_start.wait()
                server_exec(node, " ".join(command), tmux_session="azuresql_run")
                clear_table(conn_str, 'usertable')
