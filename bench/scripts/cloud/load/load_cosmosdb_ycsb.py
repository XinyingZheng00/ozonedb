    ...
    for op_cnt in map(str, operation_cnts):
        for key_size in key_sizes:
            for workload in workloads:
                ...
                server_exec(node, "rm -rf " + insert_result, tmux_session="cosmosdb_load")
                ...
                command = ["python3", "bin/ycsb", "load", db, '-threads', f'{threads}', "-s", "-P", workload_path, "-p", f"azure.account={account_name}", "-p", f"azurecosmos.primaryKey={cosmosdb_account_key}", "-p", f"azurecosmos.uri={cosmosdb_account_uri}", f"-p statusinterval=1 2>&1 | tee -a {insert_result}"]
                ycsb_t_start.wait()
