#!/bin/bash
# PLAN-cost-2 Tasks 4, 5, 6 and the 10 GB half of 7b: one 16-writer load, the h sweep,
...
step "Task 4: 10 GB trimmed load, 16 writers"
bash bench/scripts/local/load_corfu_dataset.sh --writers 16 --record-cnt $RC --log-trim
ssh -o BatchMode=yes "$SRV" 'rm -rf /mnt/corfu/load-zstd-20260827 /mnt/corfu/load-bucket-zstd-20260827; du -sh /mnt/corfu/*; df -h /tank/ssd | tail -1'

step "Task 0b check: cz at 100 MiB, 300 s"
bash $RUN --log-trim --record-cnt $RC --lru-cache-bytes 104857600 --workloads cz --writers-list 1 \
  --client-hosts "$HOSTS" --trial 1 --duration 300 --run-tag $TAG-10g-zipf
python3 $CHAINS/zipf_check.py bench/results/local/$TAG-10g-zipf
