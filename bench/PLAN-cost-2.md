...
2. Resolve the six conflicts with these rules:
   ...
   | `bench/scripts/local/load_local_ycsb_multiproc.py` | `disk_cache_corfu_settings`, `--disk-cache-*`, `--cache-warm*`, label tokens `-dc`, `-ch`, `-adm`, `-warm` | `corfu_client_corfu_settings`, `--corfu-client`, `NATIVE_LABEL_SUFFIX` in `result_label` | keep both. In `result_label` the order must be: engine, `-linearizable`, `-native`, `-lru…`, `-warm…`, `-dc…-ch…-adm` (the `visibility` order, which the disk2 result files already carry). The model's `tier_variant()` takes the tokens after `-dc<size>`, so `-native` and `-warm` must both come before `-dc` |
