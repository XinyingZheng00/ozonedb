...
from mysql_data_cache import copy_table_to_cached_db
...
def load_ycsb_t_data(node, i, ycsb_t_start, config):
    workload = config["load"]["workload"]
    operation_count = config["load"]["operationcount"]
    threadcount = config["load"]["threadcount"]
    ycsb_dir = "./bin/ycsb"
    ...
    export_file = f"{log_folder}/{workload}/load_{operation_count}_t{threadcount}_{i}.txt"
    ycsb_log_file = f"{log_folder}/{workload}/load_{operation_count}_t{threadcount}_{i}.log"

    if config["vm"]["num"] > 1:
        insertstart = int(i * operation_count / config["vm"]["num"])
        insertcount = int(operation_count / config["vm"]["num"])

        ycsb_t_command = f"""{ycsb_dir} load jdbc -P ./workloads/{workload} -threads {threadcount} -s\
 -P ./mysql.properties -p insertstart={insertstart} -p insertcount={insertcount} -p exportfile={export_file} -p statusinterval=1 2>&1 | tee {ycsb_log_file}"""

    else:
        ycsb_t_command = f"""{ycsb_dir} load jdbc -P ./workloads/{workload} -threads {threadcount} -s\
 -P ./mysql.properties -p recordcount={operation_count} -p exportfile={export_file} -p statusinterval=1 2>&1 | tee {ycsb_log_file}"""
...
def download_results(node, i, config, args):
    workload = config["load"]["workload"]
    operation_count = config["load"]["operationcount"]
    threadcount = config["load"]["threadcount"]
    ...
    download_file_from_server(
        node,
        export_file,
        f"./{args.results_dir}/results_{config['vm']['num']}vm/{workload}/mysql/load_{operation_count}_t{threadcount}_{i}.txt",
    )
    download_file_from_server(
        node,
        ycsb_log_file,
        f"./{args.results_dir}/results_{config['vm']['num']}vm/{workload}/mysql/load_{operation_count}_t{threadcount}_{i}.log",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load ycsb-t data on Azure VMs and Mysql")
    ...
    if not os.path.exists(
        f"./{args.results_dir}/results_{config['vm']['num']}vm/{config['load']['workload']}/mysql"
    ):
        pathlib.Path(
            f"./{args.results_dir}/results_{config['vm']['num']}vm/{config['load']['workload']}/mysql"
    ...
    for i, vm in enumerate(vm_list):
        ...
        ycsb_t_run.append(threading.Thread(target=load_ycsb_t_data, args=(server, i, ycsb_t_start, config)))
        result_downloader.append(threading.Thread(target=download_results, args=(server, i, config, args)))
    ...
    aggregated_results_file = f"./{args.results_dir}/results_{config['vm']['num']}vm/{config['load']['workload']}/mysql/load_aggregate_results.txt"
    with open(aggregated_results_file, "w") as file:
